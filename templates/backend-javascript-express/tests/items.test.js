...
<% if (database !== 'mongodb') { %>const ItemStore = require('../src/models/item');
<% } %>
...
<% } %>
  ...
  describe('GET /api/items', () => {
    it('should return an array of items', async () => {
      await request(app).post('/api/items').send({ title: 'Item 1' });
  ...
  describe('GET /api/items/:id', () => {
    it('should return a single item', async () => {
      const created = await request(app).post('/api/items').send({ title: 'Find Me' });
  ...
  describe('PUT /api/items/:id', () => {
    it('should update an existing item', async () => {
      const created = await request(app).post('/api/items').send({ title: 'Update Me' });
      const id = created.body.id || created.body._id;
      const res = await request(app).put(`/api/items/${id}`).send({ title: 'Updated', completed: true });
      expect(res.statusCode).toEqual(200);
  ...
  describe('DELETE /api/items/:id', () => {
    it('should delete an existing item', async () => {
      const created = await request(app).post('/api/items').send({ title: 'Delete Me' });
      const id = created.body.id || created.body._id;
