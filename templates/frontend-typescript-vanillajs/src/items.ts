...
import { fetchItems, createItem, updateItem, deleteItem, API_BASE, type Item } from './api';

export function setupItemsCrud(element: HTMLElement) {
  let items: Item[] = [];
  let loading = true;
            ...
            : items.map((item) => `
              <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.75rem; border-radius: 0.5rem; background: var(--bg-color, #f8fafc); border: 1px solid var(--border-color, #f1f5f9);">
                <label style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer; flex: 1; min-width: 0;">
                  <input type="checkbox" data-id="${item.id || (item as any)._id}" ${item.completed ? 'checked' : ''} class="item-checkbox" style="width: 1.125rem; height: 1.125rem; accent-color: #0d9488; cursor: pointer; flex-shrink: 0;" />
                ...
                <button type="button" data-id="${item.id || (item as any)._id}" class="btn-delete-item" style="background: none; border: none; color: #f43f5e; cursor: pointer; padding: 0.35rem 0.5rem; font-size: 0.875rem; line-height: 1; flex-shrink: 0;">✕</button>
              </div>
            `).join('')
          }
        </div>
  ...
  function bindEvents() {
    const form = element.querySelector<HTMLFormElement>('#crud-form');
    if (form) {
      form.addEventListener('submit', async (e) => {
        e.preventDefault();
        const input = element.querySelector<HTMLInputElement>('#item-input');
    ...
    element.querySelectorAll<HTMLInputElement>('.item-checkbox').forEach((checkbox) => {
      checkbox.addEventListener('change', async () => {
        const id = checkbox.dataset.id;
        if (!id) return;
        const item = items.find((i) => (i.id || (i as any)._id) === id);
        if (!item) return;
        try {
    ...
    element.querySelectorAll<HTMLButtonElement>('.btn-delete-item').forEach((btn) => {
