...
import { fetchItems, createItem, updateItem, deleteItem, API_BASE, type Item } from '../composables/useApi';
...
const handleDelete = async (id?: string) => {
  ...
  try {
    await deleteItem(id);
    items.value = items.value.filter((i) => (i.id || (i as any)._id) !== id);
  } catch (err: any) {
    error.value = err.message;
  }
};
  ...
  <div class="card crud-card">
    ...
    <div class="items-list">
      ...
      >
        ...
        <button
          type="button"
          @click="handleDelete(item.id || (item as any)._id)"
          class="btn-delete"
          title="Delete item"
        >
          ✕
