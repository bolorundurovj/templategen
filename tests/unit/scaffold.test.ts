...
describe('createProject', () => {
  it('returns false if directory exists', () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'Folder my-project exists. Delete or use another name.',
    );
  });
  ...
  it('clones repo when directory does not exist', () => {
    ...
    expect(shell.exec).toHaveBeenCalledWith(
      'git clone https://repo.url my-project',
    );
    expect(result).toBe(true);
  });
  ...
  it('returns false if git clone fails', () => {
    ...
    expect(result).toBe(false);
    expect(logger.error).toHaveBeenCalledWith(
      'Error cloning repository https://repo.url into my-project',
    );
  });
});
...
describe('run', () => {
  ...
  it('creates project with valid inputs', async () => {
    ...
    (REPOS as any)['fullstack-javascript-react-express-mongodb-mvc'] =
      'mocked-repo';
    const createProjectMock = jest
      .spyOn(scaffold, 'createProject')
      .mockReturnValue(true);
    const postProcessMock = jest
